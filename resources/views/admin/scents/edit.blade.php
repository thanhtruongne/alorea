...
@section('title', 'Chỉnh sửa mùi hương')
@section('page-title', 'Chỉnh sửa mùi hương')
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="mb-0">Chỉnh sửa mùi hương</h4>
        <p class="text-muted mb-0">Cập nhật thông tin: {{ $scent->name }}</p>
    </div>
...
<div class="info-badge">
    ...
    <strong>Mùi hương:</strong> {{ $scent->name }} |
    <strong>Được tạo:</strong> {{ $scent->created_at->format('d/m/Y H:i') }} |
    <strong>Cập nhật:</strong> {{ $scent->updated_at->format('d/m/Y H:i') }}
</div>

<form action="{{ route('admin.scents.update', $scent) }}" method="POST" id="scentForm">
    @csrf
    @method('PUT')
    ...
    <div class="row">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                                   value="{{ old('name', $scent->name) }}" placeholder="VD: Bergamot, Rose, Sandalwood..." id="nameInput">
                            @error('name')
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="slug" class="form-control @error('slug') is-invalid @enderror"
                                   value="{{ old('slug', $scent->slug) }}" placeholder="Tự động tạo từ tên" id="slugInput">
                            @error('slug')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <textarea name="description" class="form-control @error('description') is-invalid @enderror"
                                      rows="3" placeholder="Mô tả chi tiết về mùi hương này...">{{ old('description', $scent->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <textarea name="notes" class="form-control @error('notes') is-invalid @enderror"
                                      rows="2" placeholder="Ghi chú về cách sử dụng, đặc điểm đặc biệt...">{{ old('notes', $scent->notes) }}</textarea>
                            @error('notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="type" class="form-select @error('type') is-invalid @enderror" id="typeSelect">
                                ...
                                @foreach($types as $value => $label)
                                    <option value="{{ $value }}" {{ old('type', $scent->type) === $value ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
        ...
        <div class="col-lg-4">
                ...
                <div class="p-4">
                    <div class="form-check mb-3">
                        <input type="checkbox" name="is_active" class="form-check-input"
                               id="isActive" {{ old('is_active', $scent->is_active) ? 'checked' : '' }}>
                        <label class="form-check-label" for="isActive">
                            <i class="fas fa-power-off text-success me-1"></i>
                            Kích hoạt
                        </label>
                        <small class="form-text text-muted d-block">Mùi hương có thể được sử dụng</small>
                    </div>
                </div>
            </div>
            ...
            <div class="form-section">
                <div class="form-section-header">
                    <h5 class="mb-0"><i class="fas fa-info me-2"></i>Thông tin hiện tại</h5>
                </div>
                <div class="p-4">
                    <div class="row text-center">
                        <div class="col-4">
                            <div class="p-2 border rounded">
                                <h6 class="mb-1">{{ $scent->type_name }}</h6>
                                <small class="text-muted">{{ $scent->category_name }}</small>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="d-flex justify-content-between">
                ...
                <div class="d-flex gap-2">
                    <button type="submit" name="action" value="save" class="btn btn-primary">
                        <i class="fas fa-save me-2"></i>Cập nhật mùi hương
                    </button>
                </div>
            </div>
        </div>
...
// Show type info if type is selected
