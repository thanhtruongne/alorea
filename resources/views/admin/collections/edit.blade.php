{{-- filepath: resources/views/admin/collections/edit.blade.php --}}
@extends('admin.layout')

@section('title', 'Edit Collection')
@section('page-title', 'Chỉnh sửa bộ sưu tập')
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="mb-0">Chỉnh sửa bộ sưu tập</h4>
...
<form action="{{ route('admin.collections.update', $collection) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row">
                    ...
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            ...
                            <input type="text" name="title" class="form-control @error('title') is-invalid @enderror"
                                   value="{{ old('title', $collection->title) }}" placeholder="Nhập tiêu đề bộ sưu tập" id="titleInput">
                            @error('title')
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <input type="text" name="slug" class="form-control @error('slug') is-invalid @enderror"
                                   value="{{ old('slug', $collection->slug) }}" placeholder="Tự động tạo từ tiêu đề" id="slugInput">
                            @error('slug')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="slug-preview mt-2" id="slugPreview">URL: /collections/{{ $collection->slug }}</div>
                        </div>

                        <div class="col-md-12 mb-3">
                            <label class="form-label">Phụ đề</label>
                            <input type="text" name="sub_title" class="form-control @error('sub_title') is-invalid @enderror"
                                   value="{{ old('sub_title', $collection->sub_title) }}" placeholder="Phụ đề mô tả ngắn">
                            @error('sub_title')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <textarea id="contentEditor" name="description" class="form-control @error('description') is-invalid @enderror"
                                      rows="5" placeholder="Mô tả chi tiết về bộ sưu tập...">{{ old('description', $collection->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
            ...
            <div class="form-section">
                ...
                <div class="p-4">
                    ...
                    <div class="product-selection" id="productList">
                        @php
                            $selectedProductIds = old('products', $collection->products->pluck('id')->toArray());
                        @endphp
                    ...
                    <div class="mt-3 text-muted">
                        <span id="selectedCount">{{ count($selectedProductIds) }}</span> sản phẩm được chọn
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="form-section">
                ...
                <div class="p-4">
                    <div class="video-preview" id="videoPreview">
                        ...
                        @else
                            <i class="fas fa-video fa-2x me-2"></i>
                    ...
                    <small class="text-muted">
                        {{ $collection->video ? 'Để trống để giữ video hiện tại' : 'Chọn video' }}
                        <br>MP4, AVI, MOV (max 50MB)
                    </small>
                </div>
            </div>
            ...
            <div class="form-section">
                <div class="form-section-header">
                    <h5 class="mb-0"><i class="fas fa-info-circle me-2"></i>Thông tin bộ sưu tập</h5>
                </div>
                <div class="p-4">
                    <div class="row text-center">
                        <div class="col-12 mb-3">
                            <div class="border rounded p-2">
                                ...
                                <div class="fw-bold">{{ $collection->created_at->format('d/m/Y H:i') }}</div>
                            </div>
                        </div>
                        <div class="col-12 mb-3">
                            <div class="border rounded p-2">
                                <small class="text-muted">Cập nhật lần cuối</small>
                                <div class="fw-bold">{{ $collection->updated_at->format('d/m/Y H:i') }}</div>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="border rounded p-2">
                                <small class="text-muted">Số sản phẩm hiện tại</small>
                                <div class="fw-bold">{{ $collection->products->count() }} sản phẩm</div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between">
                ...
                <div class="d-flex gap-2">
                    <button type="button" class="btn btn-outline-danger" onclick="deleteCollection()">
                        <i class="fas fa-trash me-2"></i>Xóa bộ sưu tập
                    </button>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Bạn có chắc chắn muốn xóa bộ sưu tập <strong>"{{ $collection->title }}"</strong> không?</p>
            ...
            <div class="modal-footer">
                ...
                <form action="{{ route('admin.collections.destroy', $collection) }}" method="POST" style="display: inline;">
                    @csrf
                    @method('DELETE')
...
document.getElementById('videoInput').addEventListener('change', function() {
    ...
    if (file) {
        ...
        @if($collection->video)
            preview.innerHTML = '<video controls style="width: 100%; max-height: 200px;"><source src="{{ $collection->avatar_url }}" type="video/mp4"></video>';
        @else
